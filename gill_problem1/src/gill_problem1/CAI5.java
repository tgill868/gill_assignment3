...
public class CAI5 {
	...
	System.out.printf("\nSelect Difficulty Level of 1(least),2,3,4(most):");
	readRes = new Scanner(System.in);
	int diff= readRes.nextInt();
	System.out.printf("Select arithmetic type \n 1 for addition only \n 2 for multiplication only \n 3 for subtraction only "
			+ "\n 4 for division problems");
	int type= readRes.nextInt();
	
	for(int i=0;i<10;i++) {
	askQuestion(diff,type);
	}
	...
	public static void askQuestion(int diff, int type){
		...
		switch(type) {
		case 1:
		System.out.printf("\nHow much is %d plus %d?",rand_int1,rand_int2);
		ans = rand_int1+rand_int2;
		break;
		case 2:
		...
		break;
		case 3:
		System.out.printf("\nHow much is %d subtraction %d?",rand_int1,rand_int2);
		ans = rand_int1-rand_int2;
		break;
		case 4:
		System.out.printf("\nHow much is %d divided by %d?",rand_int1,rand_int2);
		ans = rand_int1/rand_int2;
		break;
		}
